...
def parse_timestamp(timestamp_str):
    ...
    try:
        ...
        try:
            dt_utc = datetime.strptime(timestamp_str, "%Y-%m-%dT%H:%M").replace(
                tzinfo=timezone.utc
            )
            dt_local = dt_utc.astimezone()
            return dt_local
        except ValueError:
            dt_utc = datetime.strptime(timestamp_str[:16], "%Y-%m-%dT%H:%M").replace(
                tzinfo=timezone.utc
            )
            dt_local = dt_utc.astimezone()
            return dt_local
...
def get_usage_data(db, hours=24):
    ...
    documents = list(
        collection.find(query, {"type": 1, "timestamp": 1, "ip": 1, "bookSource": 1})
    )
    print(
        f"Found {len(documents)} documents in usage collection from the last {hours} hours"
    )
    return documents
...
def group_by_hour(documents):
    ...
    hourly_unique_user_count = {
        hour: len(ips) for hour, ips in hourly_unique_ips.items()
    }
...
def get_book_clicks_by_store(documents):
    ...
    for doc in documents:
        ...
        if (
            doc.get("type") == "book_clicked"
            and "bookSource" in doc
            and doc["bookSource"]
        ):
...
def create_graph(hourly_data, hourly_total, hourly_unique_user_count):
    ...
    if all_types:
        ...
        color_map = {
            t: nice_colors[i % len(nice_colors)] for i, t in enumerate(sorted_types)
        }
        ...
        ax.set_title(
            "Usage Per Hour by Type & Unique Users (Last 24 Hours)",
            fontsize=14,
            fontweight="bold",
        )
...
def print_summary(hourly_data, hourly_total, hourly_unique_user_count, all_unique_ips):
    ...
    if hourly_unique_user_count:
        ...
        avg_unique = sum(hourly_unique_user_count.values()) / len(
            hourly_unique_user_count
        )
    ...
    for hour in sorted_hours[:20]:  # Show first 20 hours
        type_str = ", ".join(
            [f"{t}: {c}" for t, c in sorted(hourly_data[hour].items())]
        )
...
def main():
    ...
    try:
        ...
        hourly_data, hourly_total, hourly_unique_user_count, all_unique_ips = (
            group_by_hour(documents)
        )
        ...
        if store_clicks:
            ...
            sorted_stores = sorted(
                store_clicks.items(), key=lambda x: x[1], reverse=True
            )
