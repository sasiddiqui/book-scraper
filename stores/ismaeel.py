...
class IsmaeelScraper(AbstractBookScraper):
    def __init__(self):
        super().__init__(
            "https://ismaeelbooks.co.uk/", "Ismaeel Books", convert_rate=1.3
        )

    def ignore_url(self, url: str) -> bool:
        ig = ["/storage/", "/wishlist/", "/about/", "/contact/", "?add-to-cart=", "#"]
    ...
    def extract_book_info(self, soup: BeautifulSoup, url) -> dict | None:
        ...
        try:
            ...
            try:
                book_info["author"] = (
                    soup.find("th", text="المؤلف").find_next("a").text.strip()
                )
            except AttributeError:
                pass

            book_info["price"] = (
                soup.find("meta", attrs={"name": "twitter:data1"})["content"]
                .strip()
                .replace("£", "")
            )
        ...
        try:
            ...
            book_info["instock"] = (
                "in stock"
                in soup.find("meta", attrs={"name": "twitter:data2"})["content"]
                .strip()
                .lower()
            )
            try:
                book_info["publisher"] = (
                    soup.find("th", text="الناشر").find_next("a").text.strip()
                )
            except AttributeError:
                try:
                    book_info["publisher"] = (
                        soup.find("th", text="PUBLISHER").find_next("p").text.strip()
                    )
                except AttributeError:
                    pass
