...
class Buraq(AbstractBookScraper):
    ...
    def extract_book_info(self, soup, url):
        ...
        try:
            book_info["price"] = soup.find("meta", property="og:price:amount").get(
                "content"
            )
        except Exception as e:
        ...
        try:
            ...
            book_info["author"] = (
                soup.find("p", text=re.compile("Author:")).text.split(":")[1].strip()
            )
        except Exception as e:
        ...
        try:
            book_info["image"] = soup.find("meta", property="og:image:secure_url").get(
                "content"
            )
        except Exception as e:
