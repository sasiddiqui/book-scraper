...
class WooScraper:
    ...
    def test_base_url(self):
        ...
        if response.status_code == 200:
            ...
            raise ScraperError(
                f"Failed to reach {self.base_url}. Status code: {response.status_code}. Skipping scraper..."
            )
    ...
    def extract_book_info(self, product: dict) -> dict | None:
        ...
        book_info = {
            ...
            "price": product["price"],
            "image": (
                product["images"][0]["src"] if len(product["images"]) > 0 else None
            ),
        }
        return book_info
    ...
    async def crawl_product_pages(self, last_crawl_success=None):
        ...
        while True:
            ...
            r = requests.get(
                f"{self.base_url}/wp-json/wc/v3/products?per_page=100&page={page}",
                auth=(self.username, self.password),
            ).json()
            if len(r) == 0:
        ...
        return list(
            filter(
                ...
                [self.extract_book_info(product) for product in products],
            )
        )
