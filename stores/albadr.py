...
class AlBadrBooksScraper(AbstractBookScraper):
    ...
    def ignore_url(self, url) -> bool:
        ig = ["/uploads/", "/wishlist/", "/about/", "/contact/", "?", "#"]
    ...
    def extract_book_info(self, soup, url) -> dict | None:
        ...
        try:
            book_info["title"] = (
                soup.find("h1", class_="product_title entry-title").text.strip()
                + " | "
                + soup.find(
        ...
        try:
            ...
            book_info["image"] = soup.find(
                "img", class_="attachment-shop_single size-shop_single"
            )["src"]
