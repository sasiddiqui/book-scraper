...
class Kitaabun(AbstractBookScraper):
    ...
    def is_product_url(self, url):
        return (
            ...
            and "?page=" in url
            and "action=buy_now" not in url
            and "products_id" not in url
        )
    ...
    def extract_book_info(self, soup: BeautifulSoup, url) -> list | None:
        ...
        for card in soup.find_all("div", "card p-2"):
            ...
            try:
                ...
                book["price"] = (
                    float(card.find("h6").text.strip().split("&pound")[-1]) * 1.31
                )
                book["image"] = (
                    "https://kitaabun.com/shopping3/" + card.find("img")["src"]
                )
                author = card.find("a", class_="font-weight-bold").text.strip()
                if not (
                    "other" in author.lower()
                    or "Aid" in author
                    or "Motivation" in author
                ):
