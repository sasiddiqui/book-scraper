...
class AlBalagh(AbstractBookScraper):
    def __init__(self):
        ...
        self.strainer = SoupStrainer(
            ["div", "h1"],
            class_=[
                "span9 ty-product-block__left",
                "ty-product-block-title",
                "ty-product-img cm-preview-wrapper",
            ],
        )
        self.batch_size = 20
    ...
    def extract_book_info(self, soup, url) -> dict | None:
        ...
        try:
            book_info["title"] = soup.find(
                "h1", class_="ty-product-block-title"
            ).text.strip()
        ...
        try:
            book_info["author"] = (
                soup.find(
                    "span",
                    class_="ty-product-feature__label",
                    text=re.compile("Author"),
                )
                .find_next("em")
                .text.strip()
            )
        ...
        try:
            book_info["price"] = float(
                soup.find("div", class_="ty-product-block__price-actual")
                .text.strip()
                .replace("$", "")
            )
            if book_info["price"]:
        ...
        try:
            book_info["instock"] = (
                soup.find("span", class_="ty-qty-in-stock") is not None
            )
        except Exception as e:
